...
const stats = [
  { label: 'Credits Minted', value: '1,284', change: '+12 today' },
  { label: 'Credits Retired', value: '863', change: '+5 today' },
  { label: 'Active Projects', value: '47', change: '+2 this week' },
  { label: 'Tonnes CO₂ Offset', value: '24,910', change: '+320 today' },
]

const credits = [
  { id: 'VCC-001', project: 'Solar Farm Tamil Nadu', tonnes: 500, status: 'Minted', confidence: 92 },
  { id: 'VCC-002', project: 'Wind Energy Gujarat', tonnes: 320, status: 'Pending', confidence: 78 },
  { id: 'VCC-003', project: 'Mangrove Restore Kerala', tonnes: 150, status: 'Retired', confidence: 88 },
  { id: 'VCC-004', project: 'Biogas Plant Pune', tonnes: 210, status: 'Traded', confidence: 85 },
  { id: 'VCC-005', project: 'Reforestation Assam', tonnes: 430, status: 'Approved', confidence: 95 },
]
            ...
            <tbody>
              {credits.map((credit, i) => (
                <tr key={credit.id} className={`border-b border-stone-800/50 hover:bg-stone-800/30 transition-colors ${i === credits.length - 1 ? 'border-0' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${statusStyle[credit.status]}`}>
                      {credit.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
